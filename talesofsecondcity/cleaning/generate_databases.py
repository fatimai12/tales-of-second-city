...
#Assign tracts wherever possible using helper functions
preprocess()
address_to_census_tract.run()
l_stops_geocoded = geocode_l_stops()

#For df's with missing values, using (more expensive) Point to tract mapping helper
...
#Output final public services files to csv's
...
geometry_col = census_tract_shapes.iloc[:, -1]
census_tract_shapes = census_tract_shapes.iloc[:, :12]
census_tract_shapes = pd.concat([census_tract_shapes, geometry_col], axis=1)

#Merge acs with tract shape file
...
merged_demo.replace(-666666666.0, '',inplace=True) #Remove NA's
merged_demo = merged_demo[merged_demo['Total Pop (#)_2012'] != 0.0] #Remove pop 0
merged_demo.to_csv('../data/full_demo_data.csv',index=False)

#Out to GeoJSON file for use in maps
...
ps_combined_df = pd.concat([df.assign(service_type=service_type) for df, service_type in ps_with_type], ignore_index=True)
ps_combined_df.drop(columns=['ID'], inplace=True)
ps_combined_df.to_csv('../data/full_ps_data.csv',index=False)
